...
pub trait ModelsEndpointClient: fmt::Debug + Send + Sync {
    ...
    /// Returns whether this provider exposes a compatible remote model catalog.
...
impl OpenAiModelsManager {
    ...
    pub fn new(
        ...
        Self::new_with_base_catalog(
            ...
            MODEL_CACHE_FILE,
            /*use_remote_models_only*/ false,
        )
    }

    /// Construct a remote model manager with provider-specific base models and cache.
    pub fn new_with_base_catalog(
        codex_home: PathBuf,
        endpoint_client: Arc<dyn ModelsEndpointClient>,
        auth_manager: Option<Arc<AuthManager>>,
        base_model_catalog: ModelsResponse,
        cache_file_name: impl AsRef<str>,
        use_remote_models_only: bool,
    ) -> Self {
        let cache_path = codex_home.join(cache_file_name.as_ref());
        let cache_manager = ModelsCacheManager::new(cache_path, DEFAULT_MODEL_CACHE_TTL);
        let base_models = base_model_catalog.models;
        let remote_models = base_models.clone();
        Self {
            remote_models: RwLock::new(remote_models),
            etag: RwLock::new(None),
            base_models,
            use_remote_models_only,
            cache_manager,
            endpoint_client,
            auth_manager,
        }
...
impl OpenAiModelsManager {
    ...
    async fn should_refresh_models(&self) -> bool {
        self.endpoint_client.uses_codex_backend().await
            || self.endpoint_client.supports_remote_model_catalog()
    }
    ...
    async fn apply_remote_models(&self, models: Vec<ModelInfo>) {
        ...
        let should_use_remote_models_only = self.use_remote_models_only
            || (!models.is_empty()
                && models
                    .iter()
