...
#[test]
fn test_create_nvidia_nim_provider() {
    let expected_base_url = std::env::var("CODEX_NVIDIA_NIM_BASE_URL")
        .ok()
    ...
    assert_eq!(
        ...
        ModelProviderInfo {
            ...
            env_key_instructions: Some(
                "Create an API key in the NVIDIA API Catalog at https://build.nvidia.com and set NVIDIA_API_KEY."
                    .to_string()
            ),
            experimental_bearer_token: None,
            auth: None,
...
fn test_built_in_model_providers_include_nvidia_nim() {
    ...
    assert_eq!(
        providers
            .get(NVIDIA_NIM_PROVIDER_ID)
            .map(ModelProviderInfo::is_nvidia_nim),
        Some(true)
    );
}
